...
describe("DB Layer", () => {
    ...
    describe("List & Pagination", () => {
        ...
        it("should sort by downloads", () => {
            ...
            testDb
                .prepare("UPDATE assets SET downloads = 100 WHERE id = ?")
                .run(a1.id);
            expect(db.listAssets({ sort: "downloads" }).assets[0].name).toBe(
                "pop",
            );
        });
    ...
    describe("Invite Codes", () => {
        ...
        it("should reject fully-used invite code", () => {
            ...
            testDb
                .prepare("UPDATE invite_codes SET use_count = 1 WHERE code = ?")
                .run("SINGLE");
        ...
        it("should activate an invite code for a user", () => {
            ...
            expect(db.activateInviteCode("u-act", "SEAFOOD").success).toBe(
                true,
            );
            ...
            expect(
                (
                    testDb
                        .prepare(
                            "SELECT use_count FROM invite_codes WHERE code = ?",
                        )
                        .get("SEAFOOD") as any
                ).use_count,
            ).toBe(1);
        ...
        it("should reject activation if user already has invite code", () => {
            ...
            expect(db.activateInviteCode("u-al", "SEAFOOD").success).toBe(
                false,
            );
        });
    ...
    describe("Edge Cases", () => {
        it("should handle SQL injection attempt safely", () => {
            const asset = db.createAsset({
                ...
                type: "skill",
                description: "Robert'); DROP TABLE assets;--",
                version: "1.0.0",
            });
