    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (res.status === 403 && remaining === "0") {
            const resetDate = reset
                ...
                : "unknown";
            const waitMs = reset
                ? parseInt(reset) * 1000 - Date.now() + 2000
                : 61000;
            ...
            throw new Error(
                `GitHub API rate limit exceeded. Resets at ${resetDate}. Set GITHUB_TOKEN for higher limits.`,
            );
        }
        ...
        if (res.status === 429 || res.status >= 500) {
            if (attempt < maxRetries - 1) {
                ...
                console.log(
                    `  ⏳ Retrying in ${delay / 1000}s (attempt ${attempt + 1}/${maxRetries}, status ${res.status})...`,
                );
        ...
        if (remaining && parseInt(remaining) < 5) {
            console.log(
                `  ⚠️  GitHub API rate limit low: ${remaining} remaining`,
            );
        }
 ...
 */
async function fetchFlatFilesWithContent(
    owner: string,
    repo: string,
    branch: string,
): Promise<FlatFile[]> {
    ...
    const dirs = data.tree.filter(
        (item: any) => item.type === "tree" && !item.path.includes("/"),
    );
    ...
    if (blobs.length > MAX_FILES) {
        ...
        blobs.sort((a: any, b: any) => {
            const aPriority = PRIORITY_PATTERNS.some((p) => p.test(a.path))
                ? 0
                : 1;
            const bPriority = PRIORITY_PATTERNS.some((p) => p.test(b.path))
                ? 0
                : 1;
    ...
    for (const blob of blobs) {
        try {
            ...
            if (blobData?.encoding === "base64") {
                content = Buffer.from(blobData.content, "base64").toString(
                    "utf8",
                );
...
async function fetchIssues(
    ...
    const data = await ghFetch(
        `/repos/${owner}/${repo}/issues?state=all&per_page=${limit}&sort=created&direction=desc`,
    );
...
async function fetchAllRepoData(
    owner: string,
    repo: string,
): Promise<{ info: RepoInfo; content: RepoContent }> {
    ...
    const flatFiles = await fetchFlatFilesWithContent(
        owner,
        repo,
        info.defaultBranch,
    );
    ...
    return {
        info,
        content: { readme, files, flatFiles, issues, latestRelease },
    };
}
...
function detectAssetType(info: RepoInfo, readme: string): string {
    const text =
        `${info.description} ${info.topics.join(" ")} ${readme.substring(0, 1500)}`.toLowerCase();
    ...
    if (
        ...
        return "channel";
    if (
        (text.includes("websocket") || text.includes("gateway")) &&
        (text.includes("ui") ||
    ...
    if (
        ...
        (text.includes("bot") || text.includes("message"))
    )
        return "channel";
    if (
    ...
    if (
        ...
        (text.includes("openclaw") || text.includes("agent"))
    )
        return "channel";
    ...
    if (
        text.includes("fswatch") ||
        text.includes("inotify") ||
...
function getDb(): Database.Database {
    ...
    if (!colNames.has("github_language")) {
        db.exec(
            `ALTER TABLE assets ADD COLUMN github_language TEXT DEFAULT ''`,
        );
    }
    ...
    if (!colNames.has("github_synced_at")) {
        db.exec(
            `ALTER TABLE assets ADD COLUMN github_synced_at TEXT DEFAULT ''`,
        );
    }
...
function insertAsset(
    ...
    if (!description && content.readme) {
        const firstLine = content.readme
            .split("\n")
            .find((l) => l.trim() && !l.startsWith("#"));
        description =
            firstLine?.trim() || `${info.fullName} - imported from GitHub`;
    }
        ...
        ? [
              {
                  version,
                  changelog:
                      content.latestRelease.name || content.latestRelease.tag,
                  date: content.latestRelease.publishedAt,
              },
          ]
        : [
              {
  ...
  `);
    ...
    stmt.run({
        ...
        compatibility: JSON.stringify({
            models: ["Any"],
            platforms: ["OpenClaw"],
            frameworks: [info.language || "N/A"],
        }),
        files: JSON.stringify(
            content.flatFiles.length > 0
                ? content.flatFiles
                : content.files.slice(0, 200),
        ),
...
function findByGitHubUrl(db: Database.Database, url: string): string | null {
    const row = db
        .prepare(`SELECT id FROM assets WHERE github_url = ?`)
...
`);
    ...
    console.log(
        `${GITHUB_TOKEN ? "🔑 Using GitHub token" : "⚠️  No GitHub token (rate limit: 60 req/h). Set GITHUB_TOKEN for 5000/h"}`,
    );
    ...
    for (let i = 0; i < repos.length; i++) {
        ...
        try {
            ...
            if (db) {
                const existing = findByGitHubUrl(
                    db,
                    `https://github.com/${owner}/${repo}`,
                );
                if (existing && !update) {
                    console.log(
                        `  ⏭️  Already imported as ${existing}, skipping (use --update to overwrite)\n`,
                    );
                    results.push({
                        url,
                        success: true,
            ...
            console.log(
                `  ✅ ${info.fullName}: ⭐${info.stars} 🍴${info.forks} 📝${info.language} → type:${assetType}`,
            );
            console.log(
                `  📄 README: ${content.readme.length} chars | 📂 Files: ${content.files.length} | 🐛 Issues: ${content.issues.length}`,
            );
            ...
            const assetId = insertAsset(db!, info, content, {
                type: assetType,
                category,
                authorId,
                authorName,
