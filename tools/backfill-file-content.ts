...
function flattenFiles(
    nodes: FileNode[],
    prefix = "",
): { path: string; name: string; size: number; node: FileNode }[] {
    const result: {
        path: string;
        name: string;
        size: number;
...
function setFileContent(
    ...
    const dir = nodes.find(
        (n) => n.name === parts[0] && n.type === "directory",
    );
    if (dir?.children) {
...
async function main() {
    ...
    console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━`);
    console.log(
        `${GITHUB_TOKEN ? "🔑 Using GitHub token" : "⚠️  No GITHUB_TOKEN (60 req/h limit). Set GITHUB_TOKEN for 5000/h"}`,
    );
    ...
    const assets = db.prepare(query).all(...params) as {
        id: string;
        name: string;
        github_url: string;
        files: string;
    ...
    console.log(
        `📦 Found ${assets.length} GitHub-imported assets to process\n`,
    );
    ...
    for (let i = 0; i < assets.length; i++) {
        ...
        const withContent = flat.filter(
            (f) => f.node.content && f.node.content.trim(),
        );
        if (withContent.length > 0) {
            console.log(
                `[${i + 1}/${assets.length}] ⏭️  ${asset.name} — already has ${withContent.length} files with content, skipping`,
            );
            skipped++;
            continue;
        }
        ...
        // Parse GitHub URL
        const match = asset.github_url.match(
            /github\.com\/([^\/]+)\/([^\/\s#?]+)/,
        );
        if (!match) {
            console.log(
                `[${i + 1}/${assets.length}] ⚠️  ${asset.name} — can't parse GitHub URL: ${asset.github_url}`,
            );
            failed++;
            continue;
        }
        ...
        console.log(
            `[${i + 1}/${assets.length}] 📥 ${asset.name} (${owner}/${repo})`,
        );
        ...
        try {
            ...
            console.log(
                `  📂 ${flat.length} total files, ${toFetch.length} text files to fetch (~${Math.round(totalSize / 1024)}KB)`,
            );

            if (dryRun) {
                console.log(
                    `  🔍 DRY RUN — would fetch ${toFetch.length} files\n`,
                );
                continue;
            }
            ...
            // Update DB
            db.prepare(
                `UPDATE assets SET files = ?, updated_at = ? WHERE id = ?`,
            ).run(
            ...
            console.log(
                `  ✅ Fetched ${fetched}/${toFetch.length} files, DB updated\n`,
            );
            updated++;
