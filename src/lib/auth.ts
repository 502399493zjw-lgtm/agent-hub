...
import {
    peekQualificationToken,
    approveCliAuthByDevice,
    consumePendingEmailInvite,
    peekPendingEmailInvite,
...
const minimalAdapter: Adapter = {
    createUser: async (data) => {
        console.log("[auth/adapter] createUser called:", {
            email: data.email,
            name: data.name,
            hasOAuthCtx: !!_pendingOAuthContext,
        });
        ...
        const qualifyDeviceId =
            oauthCtx?.qualifyDeviceId || (await getQualificationDeviceId());
        if (qualifyDeviceId) {
    ...
    createVerificationToken: async (data) => {
        console.log("[auth/adapter] createVerificationToken:", {
    ...
    useVerificationToken: async (data) => {
        console.log("[auth/adapter] useVerificationToken:", {
            identifier: data.identifier,
            tokenLen: data.token?.length,
        });
        const result = useVerificationToken(data);
        console.log(
            "[auth/adapter] useVerificationToken result:",
            result
                ? { found: true, expires: result.expires }
                : { found: false },
        );
        return result;
    },
...
async function getInviteCodeFromCookie(): Promise<string | null> {
    try {
        ...
        return cookie?.value
            ? decodeURIComponent(cookie.value).trim().toUpperCase()
            : null;
    } catch {
        return null;
    }
...
function tryAutoApproveCliAuth(userId: string, deviceId: string | null): void {
    ...
    try {
        ...
        if (result.success) {
            console.log(
                `[auth] Auto-approved CLI auth for device ${deviceId.slice(0, 12)}... user ${userId}`,
            );
        }
    } catch (e) {
...
export const { handlers, signIn, signOut, auth } = NextAuth({
    ...
    callbacks: {
        async signIn({ user, account, profile: oauthProfile }) {
            ...
            if (provider === "resend") {
                console.log("[auth] signIn resend callback:", {
                    email: user.email,
                    userId: user.id,
                });
                ...
                if (existing?.deleted_at) {
                    console.log(
                        "[auth] signIn resend: user deleted, rejecting",
                    );
                    return false;
                }
                if (existing) {
                    ...
                    console.log(
                        "[auth] signIn resend: existing user",
                        existing.id,
                    );
                    const effectiveInviteCode =
                        inviteCode ||
                        (user.email
                            ? consumePendingEmailInvite(user.email)
                            : null);
                ...
                console.log(
                    "[auth] signIn resend: new user, allowing email send",
                );
                return true;
            }
            ...
            if (existing) {
                ...
                const freshAvatar =
                    ((oauthProfile as Record<string, unknown>)
                        ?.picture as string) ||
                    ((oauthProfile as Record<string, unknown>)
                        ?.image as string) ||
                    user.image ||
                    existing.avatar;
                console.log(
                    "[auth] signIn existing user, freshAvatar:",
                    freshAvatar?.substring(0, 60),
                );
                updateProviderInfo(
                    existing.id,
            ...
            if (inviteCode) {
                ...
                if (!validation.valid) {
                    console.log(
                        "[auth] signIn: invalid invite code, will create user without invite",
                    );
                    inviteCode = "";
                }
            }
            ...
            console.log(
                "[auth] signIn: new OAuth user, stashed context for adapter.createUser",
            );
            return true;
        },
        ...
        async session({ session, token }) {
            ...
            if (dbUser) {
                ...
                console.log(
                    "[auth] session: dbUser not found, invalidating session. provider:",
                    token.provider,
                    "providerId:",
                    token.providerId,
                );
