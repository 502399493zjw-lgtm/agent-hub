...
export function rowToAsset(row: DbRow): Asset {
    ...
    return {
        ...
        author: {
            id:
                row.author_id ||
                "u-" + row.author_name.toLowerCase().replace(/\s+/g, "-"),
        ...
        readme: row.readme,
        versions: safeParse(row.versions, []),
        dependencies: safeParse(row.dependencies, []),
        compatibility: safeParse(row.compatibility, {}),
        issueCount: row.issue_count,
        files: safeParse(row.files || "[]", []),
        configSubtype: (row.config_subtype ??
            undefined) as Asset["configSubtype"],
...
function assetToRow(a: Asset) {
    const { hubScore, hubScoreBreakdown } = calculateHubScore(
        a.downloads,
        a.rating,
        a.ratingCount,
    );
...
function ftsHasContent(): boolean {
    try {
        const row = getDb()
            .prepare("SELECT COUNT(*) as cnt FROM assets_fts")
            .get() as { cnt: number };
        return row.cnt > 0;
    } catch {
        return false;
...
export function listAssets(params: ListParams): {
    ...
    if (params.authorId) {
        conditions.push("a.author_id = @authorId");
        bindings.authorId = params.authorId;
    }
    if (params.category) {
    ...
    if (params.q) {
        ...
        if (ftsQuery && ftsHasContent()) {
            conditions.push(
                `a.rowid IN (SELECT rowid FROM assets_fts WHERE assets_fts MATCH @q)`,
            );
            bindings.q = ftsQuery;
            usingFtsRelevance = true;
        } else {
            conditions.push(
                `(a.name LIKE @q ESCAPE '\\' OR a.display_name LIKE @q ESCAPE '\\' OR a.description LIKE @q ESCAPE '\\' OR a.tags LIKE @q ESCAPE '\\')`,
            );
    ...
    const total = (
        db
            .prepare(`SELECT COUNT(*) as cnt FROM assets a ${where}`)
    ...
    switch (params.sort) {
        ...
        default:
            ...
            orderBy =
                "(ln(a.downloads + 2) / ln(2) * 3 + ln(a.github_stars + 2) / ln(2) * 0.3 + COALESCE(us.cnt, 0) * 2 + CASE WHEN has_chinese(a.display_name) OR has_chinese(a.description) OR has_chinese(a.readme) OR has_chinese(a.author_name) THEN 30 ELSE 0 END) DESC, a.updated_at DESC";
            break;
    }
...
export function getAssetById(id: string): Asset | null {
    ...
    const row = db
        .prepare(
            "SELECT a.*, COALESCE(us.cnt, 0) as user_star_count, COALESCE(uauth.reputation, 0) as author_reputation FROM assets a LEFT JOIN (SELECT asset_id, COUNT(*) as cnt FROM user_stars WHERE asset_id = ? GROUP BY asset_id) us ON us.asset_id = a.id LEFT JOIN users uauth ON uauth.id = a.author_id WHERE a.id = ?",
        )
        .get(id, id) as DbRow | undefined;
...
export function createAsset(data: {
    ...
    const authorId =
        data.authorId || "u-" + authorName.toLowerCase().replace(/\s+/g, "-");
    ...
    if (data.authorId && !data.skipCoinReward) {
        ...
        addCoins(
            data.authorId,
            "shrimp_coin",
            SHRIMP_COIN_EVENTS.publish_asset,
            "publish_asset",
            id,
        );
    }
...
export function updateAsset(
    ...
    if ((data as { files?: unknown }).files !== undefined) {
        updates.push("files = @files");
        bindings.files = JSON.stringify((data as { files: unknown }).files);
    }
    if (data.githubUrl !== undefined) {
        updates.push("github_url = @ghu");
        bindings.ghu = data.githubUrl;
    }
    if (data.githubStars !== undefined) {
        updates.push("github_stars = @ghs");
        bindings.ghs = data.githubStars;
    }
    if (data.githubForks !== undefined) {
        updates.push("github_forks = @ghf");
        bindings.ghf = data.githubForks;
    }
    if (data.githubLanguage !== undefined) {
        updates.push("github_language = @ghl");
        bindings.ghl = data.githubLanguage;
    }
    if (data.githubLicense !== undefined) {
        updates.push("github_license = @ghli");
        bindings.ghli = data.githubLicense;
    }
    if (data.githubUrl !== undefined) {
        updates.push("github_synced_at = @ghsa");
        bindings.ghsa = new Date().toISOString();
    }
    ...
    db.prepare(`UPDATE assets SET ${updates.join(", ")} WHERE id = @id`).run(
        bindings,
    );
    return rowToAsset(
        db.prepare("SELECT * FROM assets WHERE id = ?").get(id) as DbRow,
    );
}

export function deleteAsset(id: string): boolean {
    return (
        getDb().prepare("DELETE FROM assets WHERE id = ?").run(id).changes > 0
    );
}
...
export function incrementDownload(
    ...
    const result = db
        .prepare("UPDATE assets SET downloads = downloads + 1 WHERE id = ?")
        .run(assetId);
    if (result.changes === 0) return null;

    const asset = db
        .prepare(
            "SELECT author_id, version, downloads FROM assets WHERE id = ?",
        )
        .get(assetId) as
        | { author_id: string; version: string; downloads: number }
        | undefined;
    ...
    if (userId) {
        ...
        if (hasEnoughCoins(userId, 1)) {
            addCoins(
                userId,
                "shrimp_coin",
                SHRIMP_COIN_EVENTS.install_asset,
                "install_asset",
                assetId,
            );
        }
        ...
        const existing = db
            .prepare(
                "SELECT last_version FROM user_installs WHERE user_id = ? AND asset_id = ?",
            )
            .get(userId, assetId) as { last_version: string } | undefined;
        const now = new Date().toISOString();

        if (!existing) {
            // First install ever — reward author
            db.prepare(
                "INSERT INTO user_installs (user_id, asset_id, last_version, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
            ).run(userId, assetId, asset.version, now, now);
            if (asset.author_id && asset.author_id !== userId) {
                addCoins(
                    asset.author_id,
                    "reputation",
                    USER_REP_EVENTS.asset_installed,
                    "asset_installed",
                    assetId,
                );
                addCoins(
                    asset.author_id,
                    "shrimp_coin",
                    SHRIMP_COIN_EVENTS.asset_installed,
                    "asset_installed",
                    assetId,
                );
            }
            ...
            db.prepare(
                "UPDATE user_installs SET last_version = ?, updated_at = ? WHERE user_id = ? AND asset_id = ?",
            ).run(asset.version, now, userId, assetId);
            if (asset.author_id && asset.author_id !== userId) {
                addCoins(
                    asset.author_id,
                    "reputation",
...
export function listAssetsCompact(params: ListParams & { tag?: string }): {
    ...
    switch (params.sort) {
        ...
        default:
            ...
            orderBy =
                "(ln(a.downloads + 2) / ln(2) * 3 + ln(a.github_stars + 2) / ln(2) * 0.3 + CASE WHEN has_chinese(a.display_name) OR has_chinese(a.description) OR has_chinese(a.readme) OR has_chinese(a.author_name) THEN 30 ELSE 0 END) DESC, a.updated_at DESC";
            break;
    }
    ...
    return {
        assets: rows.map((r) => ({
            ...
            category: r.category,
            githubStars: r.github_stars || 0,
            totalStars: (r.github_stars || 0) + (r.user_star_count || 0),
        })),
        total,
        page,
        pageSize,
...
export function getAllTags(): { name: string; count: number }[] {
    return getCached("allTags", 60_000, () => {
        const db = getDb();
        const rows = db.prepare("SELECT tags FROM assets").all() as {
            tags: string;
        }[];
...
export function getAssetManifest(
    id: string,
): { id: string; manifest: Record<string, unknown> } | null {
    const row = getDb()
        .prepare("SELECT id, manifest FROM assets WHERE id = ?")
        .get(id) as { id: string; manifest: string } | undefined;
    if (!row) return null;
    return { id: row.id, manifest: safeParse(row.manifest || "{}", {}) };
}

export function updateAssetManifest(
    id: string,
    manifest: Record<string, unknown>,
): boolean {
    return (
        getDb()
            .prepare("UPDATE assets SET manifest = ? WHERE id = ?")
            .run(JSON.stringify(manifest), id).changes > 0
    );
}

export function getAssetReadme(
    id: string,
): {
    name: string;
    displayName: string;
    ...
    const row = getDb()
        .prepare(
            "SELECT name, display_name, readme, version FROM assets WHERE id = ?",
        )
        .get(id) as
        | {
              name: string;
...
export function getAssetsByIds(ids: string[]): AssetCompact[] {
    ...
    const rows = db
        .prepare(
            `SELECT a.id, a.name, a.display_name, a.type, a.description, a.tags, a.downloads, a.rating, a.author_name, a.author_id, a.author_avatar, a.version, a.install_command, a.updated_at, a.category, a.github_stars, COALESCE(us.cnt, 0) as user_star_count, COALESCE(uauth.reputation, 0) as author_reputation FROM assets a LEFT JOIN (SELECT asset_id, COUNT(*) as cnt FROM user_stars GROUP BY asset_id) us ON us.asset_id = a.id LEFT JOIN users uauth ON uauth.id = a.author_id WHERE a.id IN (${placeholders})`,
        )
        .all(...ids) as {
        id: string;
        name: string;
...
export function getTrendingAssets(
    period: string,
    limit: number = 10,
): AssetCompact[] {
    const db = getDb();
    const rows = db
        .prepare(
            `SELECT a.id, a.name, a.display_name, a.type, a.description, a.tags, a.downloads, a.rating, a.author_name, a.author_id, a.author_avatar, a.version, a.install_command, a.updated_at, a.category, a.github_stars, COALESCE(us.cnt, 0) as user_star_count, COALESCE(uauth.reputation, 0) as author_reputation FROM assets a LEFT JOIN (SELECT asset_id, COUNT(*) as cnt FROM user_stars GROUP BY asset_id) us ON us.asset_id = a.id LEFT JOIN users uauth ON uauth.id = a.author_id ORDER BY a.downloads DESC, a.updated_at DESC LIMIT ?`,
        )
        .all(Math.min(limit, 50)) as {
        id: string;
        name: string;
        display_name: string;
        type: string;
...
export function getAssetVersions(assetId: string): AssetVersionInfo[] | null {
    const db = getDb();
    const row = db
        .prepare("SELECT versions FROM assets WHERE id = ?")
        .get(assetId) as { versions: string } | undefined;
    if (!row) return null;
...
export function getDependentAssets(assetId: string): Asset[] {
    ...
    const rows = db
        .prepare(`SELECT * FROM assets WHERE dependencies LIKE ?`)
        .all(`%${assetId}%`) as DbRow[];
    return rows.map(rowToAsset).filter((a) => a.dependencies.includes(assetId));
}
...
export function resolveByHash(hash: string): HashResolveResult[] {
    const db = getDb();
    const rows = db
        .prepare("SELECT id, name, version, files FROM assets")
        .all() as {
        id: string;
        name: string;
        version: string;
    ...
    for (const row of rows) {
        const files = safeParse(row.files || "[]", []) as {
            path: string;
            sha256?: string;
        }[];
