...
const TABS: {
    ...
    {
        key: "experience_template",
        label: "经验/合集",
        types: ["experience", "template"],
        desc: "实践方案、配置思路、或多资产组合包",
    },
    {
        key: "skill",
        label: "技能",
        type: "skill",
        desc: "Agent 可直接学习的能力包，含提示词与脚本",
    },
    {
        key: "plugin",
        label: "插件",
        type: "plugin",
        desc: "代码级扩展，为 Agent 接入新能力和服务",
    },
    {
        key: "trigger",
        label: "触发器",
        type: "trigger",
        desc: "定义触发策略，可仅提供事件源，也可附带触发后的任务描述",
    },
    {
        key: "channel",
        label: "通信器",
        type: "channel",
        desc: "消息渠道适配器，让 Agent 接入更多平台",
    },
];
...
function ActivityFeed({
    ...
    activities: StatsData["recentActivity"];
}) {
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm text-white/80 leading-relaxed">
                                ...
                                (item.authorAvatar.startsWith("http://") ||
                                    item.authorAvatar.startsWith("https://") ||
                                    item.authorAvatar.startsWith("/api/") ||
                                    item.authorAvatar.startsWith("data:")) ? (
                                    <img
                                        src={item.authorAvatar}
                                        alt={item.authorName}
                                        className="w-5 h-5 rounded-full inline-block mr-1.5 align-text-bottom"
                                    />
                                ) : item.authorAvatar ? (
                                    <span className="mr-1">
                                        {item.authorAvatar}
    ...
    return (
        <div className="relative">
            ...
            <section className="relative overflow-hidden min-h-[85vh] flex items-center section-light">
                ...
                <div className="absolute right-0 top-[4%] bottom-[100px] w-[42.5%] hidden md:block pointer-events-none overflow-hidden z-0">
                    ...
                    <div
                        ...
                        style={{
                            background:
                                "linear-gradient(to right, #ffffff 0%, #ffffff 5%, transparent 45%)",
                        }}
                    />
                    <div
                        className="absolute inset-0"
                        style={{
                            background:
                                "linear-gradient(to top, #ffffff 0%, #ffffff 5%, transparent 35%)",
                        }}
                    />
                    <div
                        className="absolute inset-0"
                        ...
                        <div className="animate-fade-in-up">
                            ...
                            >
                                <div className="flex-1 min-w-0 overflow-hidden">
                                    <code className="text-xs font-mono text-muted/50 whitespace-nowrap block overflow-x-auto scrollbar-hide pointer-events-none">
                                        帮我安装技能，命令行指令是 curl -sL
                                        https://openclawmp.cc/api/v1/assets/s-65623b82a16d719e/download
                                        -o /tmp/_oc_pkg.zip &amp;&amp; unzip -oq
                                        /tmp/_oc_pkg.zip -d
                                        ~/.openclaw/skills/openclawmp &amp;&amp;
                                        rm /tmp/_oc_pkg.zip
                                    </code>
                                </div>
                        ...
                        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                {[
                                    ...
                                    >
                                        <p className="text-[10px] uppercase tracking-widest text-muted font-display">
                                            {stat.label}
                                        </p>
            ...
            {stats && stats.topDevelopers.length > 0 && (
                ...
                >
                    <div
                        className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${developers.isVisible ? "animate-fade-in-up" : "opacity-0"}`}
                    >
                        <div className="mb-6">
                                ...
                                >
                                    <div className="flex items-center gap-3 px-4 py-3 rounded-full border border-card-border bg-white hover:border-blue/30 hover:shadow-sm transition-all duration-150 min-w-[200px] sm:min-w-[220px]">
                                        ...
                                        (dev.avatar.startsWith("http://") ||
                                            dev.avatar.startsWith("https://") ||
                                            dev.avatar.startsWith("data:")) ? (
                                            <img
                                                src={dev.avatar}
                                                alt={dev.name}
                                        ...
                                        <div className="flex flex-col">
                                            <p className="text-sm font-semibold truncate group-hover:text-blue transition-[color] duration-150">
                                                {dev.name}
                                            </p>
            ...
            >
                <div
                    className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${featured.isVisible ? "animate-fade-in-up" : "opacity-0"}`}
                >
                    <div className="flex items-end justify-between mb-12">
                        <div>
                            <p className="font-display text-xs uppercase tracking-[0.2em] text-muted mb-3">
                                Featured Assets
                            </p>
                        ...
                        <Link
                            href={
                                currentTab?.type
                                    ? `/explore?type=${currentTab.type}`
                                    : currentTab?.types
                                      ? `/explore?type=${currentTab.types[0]}`
                                      : "/explore"
                            }
                    ...
                    <div className="flex flex-col md:flex-row gap-8">
                                ...
                                {TABS.map((tab) => {
                                    const count = tab.types
                                        ? tab.types.reduce(
                                              (sum, t) =>
                                                  sum +
                                                  (stats?.typeCounts?.[t] ?? 0),
                                              0,
                                          )
                                        : tab.type
                                          ? (stats?.typeCounts?.[tab.type] ?? 0)
                                          : 0;
                                    return (
                                        <button
                                            key={tab.key}
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm text-muted mb-4 md:hidden">
                                {currentTab?.desc}
                            </p>
                                        ...
                                        {currentTabAssets
                                            ...
                                            .map((asset) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div
                                                            className="group px-5 py-5 hover:bg-surface/50 transition-colors duration-150 border-b"
                                                            style={{
                                                                borderColor:
                                                                    "#e5e7eb",
                                                            }}
                                                            ...
                                                            <div className="flex items-center gap-2 mb-2">
                                                                <div className="flex-shrink-0">
                                                                    {asset
                                                                        .author
                                                                        .avatar &&
                                                                    (asset.author.avatar.startsWith(
                                                                        "http",
                                                                    ) ||
                                                                        asset.author.avatar.startsWith(
                                                                            "/api/avatars/",
                                                                        ...
                                                                        <img
                                                                            ...
                                                                            className="w-5 h-5 rounded-full object-cover border border-card-border"
                                                                        />
                                                                    ) : (
                                                                        ...
                                                                        .map(
                                                                            (
                                                                                ...
                                                                                <span
                                                                                    ...
                                                                                    className="text-[10px] px-2 py-0.5 rounded-full bg-surface text-muted border border-card-border"
                                                                                >
                                                                                    {
                                                                                        tag
                                                                                    }
            ...
            {stats && stats.recentActivity.length > 0 && (
                ...
                >
                    <div
                        className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${activity.isVisible ? "animate-fade-in-up" : "opacity-0"}`}
                    >
                        <div className="flex items-end justify-between mb-12">
                            <div>
                                <p className="font-display text-xs uppercase tracking-[0.2em] text-white/60 mb-3">
                                    Live Feed
                                </p>
            ...
            <section ref={cta.ref} className="section-light py-24 md:py-32">
                <div
                    className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${cta.isVisible ? "animate-fade-in-up" : "opacity-0"}`}
                >
                    <div className="text-center">
                        <p className="font-display text-xs uppercase tracking-[0.2em] text-muted mb-6">
                            Join the Community
                        </p>
                        <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 text-foreground tracking-tight">
                            让你的 Agent
                            <br className="hidden sm:block" /> 加入社区
                        </h2>
