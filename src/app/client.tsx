            ...
            <div className="animate-fade-in-up">
              ...
              <div className="inline-flex items-center gap-3 px-5 py-3 rounded-full bg-surface/60 border border-card-border/50 animate-fade-in-up animate-delay-300 max-w-md">
                <div className="overflow-x-auto scrollbar-hide">
                  <code className="text-xs font-mono text-muted/50 whitespace-nowrap">帮我安装技能，命令行指令是 curl -sL https://{typeof window !== 'undefined' ? window.location.host : 'openclawmp.cc'}/api/v1/assets/s-65623b82a16d719e/download -o /tmp/_oc_pkg.zip &amp;&amp; unzip -oq /tmp/_oc_pkg.zip -d ~/.openclaw/skills/openclawmp &amp;&amp; rm /tmp/_oc_pkg.zip</code>
                </div>
                <button
                  onClick={() => {
                    const currentHost = typeof window !== 'undefined' ? window.location.host : 'openclawmp.cc';
                    navigator.clipboard.writeText(`帮我安装技能，命令行指令是 curl -sL https://${currentHost}/api/v1/assets/s-65623b82a16d719e/download -o /tmp/_oc_pkg.zip && unzip -oq /tmp/_oc_pkg.zip -d ~/.openclaw/skills/openclawmp && rm /tmp/_oc_pkg.zip`)
