...
export async function GET(request: NextRequest) {
    ...
    if (!code || !deviceId) {
        return NextResponse.json(
            { success: false, error: "Missing code or deviceId" },
            { status: 400 },
        );
    }
    ...
    if (!result) {
        return NextResponse.json(
            { success: false, error: "Auth request not found" },
            { status: 404 },
        );
    }
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (!dbUser?.invite_code) {
            return NextResponse.json(
                { success: false, error: "需要先激活邀请码才能授权设备" },
                { status: 403 },
            );
        }
        ...
        const result = approveCliAuthRequest(
            code.toUpperCase(),
            session.user.id,
        );
        if (!result.success) {
