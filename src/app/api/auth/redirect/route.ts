...
export async function GET(req: NextRequest) {
    ...
    if (!qt || !provider) {
        return NextResponse.json(
            { error: "Missing qt or provider parameter" },
            { status: 400 },
        );
    }
    ...
    if (provider === "email") {
        ...
        const codeParam =
            peek.valid && peek.inviteCode
                ? `&code=${encodeURIComponent(peek.inviteCode)}`
                : "";
        return NextResponse.redirect(
            `${baseUrl}/register?qt=${qt}&provider=email${codeParam}`,
        );
    }
    ...
    try {
        ...
        const signInRes = await fetch(
            `${internalBase}/api/auth/signin/${provider}`,
            {
                method: "POST",
                headers: {
        ...
        if (
            ...
            return NextResponse.json(
                { error: "OAuth provider configuration error", location },
                { status: 500 },
            );
        }
        ...
        headers.append(
            "set-cookie",
            `qualification_token=${qt}; Path=/; Max-Age=600; HttpOnly; SameSite=Lax; Secure`,
        );
        ...
        if (peek.valid && peek.inviteCode) {
            headers.append(
                "set-cookie",
                `invite_code=${encodeURIComponent(peek.inviteCode)}; Path=/; Max-Age=600; SameSite=Lax; Secure`,
            );
            console.log(
                "[auth/redirect] invite_code cookie set for code:",
                peek.inviteCode,
            );
        } else {
            console.warn(
                "[auth/redirect] No valid invite_code from qt:",
                qt,
                "peek:",
                JSON.stringify(peek),
            );
