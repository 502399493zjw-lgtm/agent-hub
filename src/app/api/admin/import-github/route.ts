...
async function ghFetch(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (res.status === 403 && rl.remaining === 0) {
            ...
            throw new Error(
                `GitHub rate limit exceeded. Resets at ${new Date(rl.resetAt * 1000).toISOString()}`,
            );
        }
...
async function fetchFilesBatched(
    ...
    if (prefix) {
        items = items
            .filter((i: any) => i.path.startsWith(prefix))
            .map((i: any) => ({ ...i, path: i.path.slice(prefix.length) }));
    }
    ...
    let blobs = items.filter((item: any) => {
        ...
        if (isBinaryFile(item.path) && fileSize <= MAX_SINGLE_BINARY)
            return true;
    ...
    for (const blob of blobsFromOffset) {
        ...
        try {
            ...
            if (knownBinary) {
                ...
                if (
                    totalBytes + buf.length > MAX_BYTES_PER_BATCH &&
                    fetchedCount > 0
                )
                    break;
                ...
                if (
                    totalBytes + contentBytes > MAX_BYTES_PER_BATCH &&
                    fetchedCount > 0
                )
                    break;
...
function detectAssetType(info: RepoInfo, readme: string): string {
    ...
    if (
        ...
        (text.includes("ui") || text.includes("desktop"))
    )
        return "channel";
    if (
...
function findByGithubUrl(
    githubUrl: string,
): { id: string; name: string } | null {
    const db = getDb();
    const row = db
        .prepare("SELECT id, name FROM assets WHERE github_url = ?")
        .get(githubUrl) as { id: string; name: string } | undefined;
    return row || null;
}
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // ── 5. Asset type ──
        const assetType =
            (body.type as string) || detectAssetType(info, readme);
        ...
        if (!skipFiles) {
            batchResult = await fetchFilesBatched(
                owner,
                repoName,
                info.defaultBranch,
                offset,
        ...
        let user = findUserByProvider(
            "github",
            String(info.avatarUrl ? info.owner : owner),
        );
        // Try by owner login
        ...
        if (!user) {
            ...
            getDb()
                .prepare("UPDATE users SET invite_code = ? WHERE id = ?")
                .run("ADMIN_IMPORT", user.id);
        }
        ...
        if (existing && (shouldUpdate || offset > 0)) {
            ...
            if (offset > 0 && batchResult.files.length > 0) {
                const db = getDb();
                const row = db
                    .prepare("SELECT files FROM assets WHERE id = ?")
                    .get(assetId) as { files: string } | undefined;
                let existingFiles: FlatFile[] = [];
                try {
                    existingFiles = JSON.parse(row?.files || "[]");
                } catch {
                    /* empty */
                }
            ...
            assetName = (subPath ? displayName : info.repo)
                .toLowerCase()
                .replace(/[^a-z0-9-]/g, "-");
            ...
            if (!description && readme) {
                ...
                description =
                    firstLine?.trim() ||
                    `${info.fullName} - imported from GitHub`;
            }
            if (!description)
                description = `${info.fullName} - imported from GitHub`;
        ...
        if (batchResult.remaining === 0) {
            ...
            if (allFiles.length > 0) {
                ...
                const missingBinaries = allFiles.filter(
                    (f) => f.binary && !allBinaryBuffers.has(f.path),
                );
                if (missingBinaries.length > 0) {
                    // Re-fetch missing binary blobs from GitHub
                    const treeData = await ghFetch(
                        `/repos/${owner}/${repoName}/git/trees/${info.defaultBranch}?recursive=1`,
                    );
                    if (treeData?.tree) {
                        for (const missing of missingBinaries) {
                            const treeItem = treeData.tree.find(
                                (t: any) =>
                                    t.path === missing.path ||
                                    t.path ===
                                        (subPath
                            ...
                            if (treeItem?.sha) {
                                try {
                                    const blobData = await ghFetch(
                                        `/repos/${owner}/${repoName}/git/blobs/${treeItem.sha}`,
                                    );
                                    if (blobData?.content) {
                                        allBinaryBuffers.set(
                                            missing.path,
                                            Buffer.from(
                                                blobData.content,
                                                "base64",
                ...
                data: {
                    ...
                    package: packageInfo
                        ...
                        : batchResult.remaining > 0
                          ? {
                                pending: true,
                                hint: `Call again with offset:${batchResult.nextOffset} to continue`,
                            }
                          : null,
                    issues: {
                        total: issues.length,
        ...
        console.error(
            "Admin import-github error:",
            message.replace(/token\s+\S+/gi, "token [REDACTED]"),
        );
        return NextResponse.json(
            {
                success: false,
                error: `Failed to import: ${message.slice(0, 200)}`,
            },
            { status: 500 },
        );
    }
