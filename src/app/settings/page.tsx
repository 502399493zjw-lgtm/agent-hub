...
function SettingsContent() {
    ...
    const handleRemoveDevice = async (deviceId: string) => {
        if (
            !confirm(
                "确定要解绑此设备？解绑后该设备上的 Agent 将无法再以你的身份操作。",
            )
        )
            return;
                                    ...
                                    <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-surface border border-card-border w-fit">
                                        ...
                                        {user.provider === "feishu" && (
                                            <>
                                                ...
                                                >
                                                    <path
                                                        d="M3 17.6L9.3 6.5c.3-.5.9-.7 1.4-.4l8.7 5c.5.3.7.9.4 1.4L13.5 23.6c-.3.5-.9.7-1.4.4l-8.7-5c-.5-.3-.7-.9-.4-1.4z"
                                                        fill="#3370FF"
                                                    />
                                                    <path
                                                        d="M7.5 3.8L13.8 0c.5-.3 1.1-.1 1.4.4l3.3 5.7c.3.5.1 1.1-.4 1.4l-6.3 3.6c-.5.3-1.1.1-1.4-.4L7.1 5.2c-.3-.5-.1-1.1.4-1.4z"
                                                        fill="#00D6B9"
                                                    />
                                                </svg>
                                                <span className="text-sm text-muted">
                        ...
                        <section className="bg-white border border-card-border rounded-lg p-6 space-y-6">
                            ...
                            <div className="mt-6 p-4 rounded-lg bg-amber-50/60 border border-amber-200/50">
                                ...
                                <p className="text-xs text-muted mb-3">
                                    安装技能并绑定设备后，将以下指令复制发给
                                    Agent：
                                </p>
                                <div className="flex items-start gap-2 bg-white rounded-lg border border-card-border p-3">
                                    <code className="text-xs text-foreground break-all whitespace-pre-wrap flex-1">
                                        帮我安装以下经验：{"\n"}openclawmp
                                        install
                                        experience/@u-3ce5e3aff0c34baaa034/self-evolution
                                        {"\n"}openclawmp install
                                        experience/@u-3ce5e3aff0c34baaa034/openclawmp-contributor
                                    </code>
                                    <button
                                        onClick={() => {
                                            navigator.clipboard.writeText(
                                                ...
                                                {userCodes.map((c) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div className="flex items-center gap-2">
                                                                <span
                                                                    className={`font-mono text-sm font-semibold ${isUsed ? "text-gray-400 line-through" : "text-blue"}`}
                                                                >
                                                                    {c.code}
                                                                </span>
                                                                {isUsed && (
